...
//         const allStories = await Story.find({}).populate("author", "name username profileImage")
//         const stories = user.followers.map(follower=> allStories.filter(story=> story.author?._id.toString() == follower._id.toString()))
//         return res.status(200).json(stories)
//     }
//     catch(error){
//         return res.status(500).json({message : `Error in getAllStories ${error}`})
//     }
// }
...
export const getStoryByUsername = async(req, res)=>{
    try{
        ...
        if(!user) return res.status(400).json({message : "User does not exist"})
        const story = await Story.findById({author : user._id})
        if(!story) return res.status(400).json({message : "Story does not found"})
        await story
            .populate("author", "name username profileImage")
            .populate("viewrs", "name username profileImage")
        return res.status(200).json(story)
    }
    catch(error){
